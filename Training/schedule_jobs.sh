...
submit_job_array() {
    ...
    if [ "$is_cluster" -eq 1 ]; then
        ...
        dir=embeddings/${dataset}/logs/${metafolder}_${batch_name}
        mkdir -p "$dir"
        ...
        # Local execution: call dicee for each data_path
        echo "➡️ Running locally for $batch_file" | tee -a "$log_file"
        while read -r data_path; do
            echo "Processing $data_path" | tee -a "$log_file"
            dicee --path_single_kg "$data_path" \
                  ...
                  --optim Adopt
            if [ $? -ne 0 ]; then
                echo "❌ dicee failed for $data_path" | tee -a "$log_file"
            fi
            ((submitted_jobs++))
        done < "$batch_file"
    fi
...
for dataset_folder in ../data/domain_dataset/*/; do
    ...
    # Filter out literal pattern when no matches
    ...
    for bf in "${batch_files[@]}"; do
        ...
        if [ "$is_cluster" -eq 1 ]; then wait_for_completion; fi
        submit_job_array "$bf"
    done
